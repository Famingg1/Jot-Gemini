...
class MeetingManager extends EventEmitter {
  ...
  levels(payload) { if(payload.id===this.active?.id&&Number.isFinite(payload.visualLevel)){this.emit('visual-level',Math.max(0,Math.min(1,payload.visualLevel)));return;} if (payload.id === this.active?.id) this.emit('levels', { id: payload.id, mic: Math.max(0,Math.min(1,Number(payload.mic) || 0)), system: Math.max(0,Math.min(1,Number(payload.system) || 0)), durationMs: Math.max(0, Number(payload.durationMs) || 0), interruptedSource: ['mic','system'].includes(payload.interruptedSource) ? payload.interruptedSource : null }); }
  async stop() {
