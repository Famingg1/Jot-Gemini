...
test('scribe meetings go up as one request with consistent speakers and map provider errors', async t => {
  const storage=setup(t); const id=record(storage,700); const requests=[];
  const fetchImpl=async(url,init)=>{ requests.push(init.body); if(requests.length===1) return { ok:false, status:401, json:async()=>({detail:{message:'bad key'}}) }; return { ok:true, status:200, json:async()=>({ language_code:'nld', audio_duration_secs:700, text:'Hoi. Ja.', words:[{text:'Hoi.',start:1,end:1.4,type:'word',speaker_id:'speaker_0'},{text:'Ja.',start:650,end:650.5,type:'word',speaker_id:'speaker_1'}] }) }; };
  let summaries=0; const clientFactory=async()=>({models:{generateContent:async()=>{summaries++; return {text:JSON.stringify({title:'Scribe test',summary:'S',decisions:[],actions:[]})};}}});
  const tx=new MeetingTranscriber({storage,getApiKey:()=> 'gemini',getElevenLabsKey:()=> 'sk_test',getSettings:()=>({meetingModel:'scribe_v2',language:'nl-NL'}),clientFactory,fetchImpl});
  await tx.process(id); assert.equal(storage.get(id).state,'saved'); assert.equal(storage.get(id).error.code,'auth'); assert.match(storage.get(id).error.message,/ElevenLabs/); assert.equal(summaries,0);
  await tx.process(id); const meeting=storage.get(id); assert.equal(meeting.state,'ready'); assert.equal(requests.length,2);
  assert.equal(requests[1].get('file').size, 44 + RATE*2*700, 'whole meeting in one wav'); assert.equal(requests[1].get('language_code'),'nl');
  assert.deepEqual(meeting.transcript.segments.map(s=>[s.speakerId,s.startMs,s.text]),[['b0-speaker_0',1000,'Hoi.'],['b0-speaker_1',650000,'Ja.']]);
  assert.equal(Object.keys(storage.manifest(id).batches||{}).length,1); assert.equal(summaries,1);
});
