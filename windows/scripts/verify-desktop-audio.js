...
function amplitude(pcm,hz){let re=0,im=0;for(let n=0;n<pcm.length/2;n++){const value=pcm.readInt16LE(n*2);re+=value*Math.cos(2*Math.PI*hz*n/16000);im+=value*Math.sin(2*Math.PI*hz*n/16000);}return Math.hypot(re,im)/(pcm.length/2)*2;}
...
async function run(){
 ...
 try{
   ...
   assert.ok(amplitudes[440]>50,'Edge is audible in saved app recording');assert.ok(amplitudes[660]>50,'New Opera app is audible');for(const hz of [1200,1600,2000,2400])assert.ok(amplitudes[hz]<5,'Excluded music frequency '+hz);
   const result={ok:true,packaged,amplitudes,flows:report.flows,profile:root,uploaded:false};fs.writeFileSync(path.join(output,'audio-verification.json'),JSON.stringify(result,null,2));console.log(result);
